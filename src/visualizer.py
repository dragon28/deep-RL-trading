...
class Visualizer:
    ...
    def plot_episodes(self, 
        ...
        if explored_total_rewards is not None:
            ...
            #ma = pd.rolling(np.array(explored_total_rewards), window=MA_window, min_periods=1).median()
            ma = pd.DataFrame(np.array(explored_total_rewards)).rolling(window=MA_window, min_periods=1).median()
            #std = pd.rolling_std(np.array(explored_total_rewards), window=MA_window, min_periods=3)
            std = pd.DataFrame(np.array(explored_total_rewards)).rolling(window=MA_window, min_periods=3).std()
        ...
        ma = pd.DataFrame(np.array(safe_total_rewards)).rolling(window=MA_window, min_periods=1).median()
        #std = pd.rolling_std(np.array(safe_total_rewards), window=MA_window, min_periods=3)
        std = pd.DataFrame(np.array(safe_total_rewards)).rolling(window=MA_window, min_periods=3).std()
