...
class Sampler:
    ...
    def load_db(self, fld):
        ...
        #self.db = pickle.load(open(os.path.join(str("..//" + fld), 'db.pickle'),'rb'))
        param = json.load(open(os.path.join(fld, 'param.json'),'rb'))
        #param = json.load(open(os.path.join(str("..//" + fld), 'param.json'),'rb'))
        self.i_db = 0
    ...
    def build_db(self, n_episodes, fld):
        ...
        #pickle.dump(db, open(os.path.join(str("..//" + fld), 'db.pickle'),'wb'))
        param = {'n_episodes':n_episodes}
        for k in self.attrs:
            param[k] = getattr(self, k)
        json.dump(param, open(os.path.join(fld, 'param.json'),'w'))
        #json.dump(param, open(os.path.join(str("..//" + fld), 'param.json'),'w'))
